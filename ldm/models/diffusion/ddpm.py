...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 embedding_density_reg_weight=0.,
        ...
        self.embedding_density_reg_weight = embedding_density_reg_weight
...
class LatentDiffusion(DDPM):
    ...
    def instantiate_embedding_manager(self, config, embedder):
        if self.model.conditioning_key == 'crossattn_layerwise':
            placeholder_strings = config['params']['placeholder_strings']
            initializer_words = config['params']['initializer_words']
            self.placeholder_token = placeholder_strings[0]
            self.layer_tokens = [self.placeholder_token+hex(i) for i in range(16)]
            config['params']['placeholder_strings'] = self.layer_tokens
            config['params']['initializer_words'] = [initializer_words[0]] * 16
    ...
    def get_learned_conditioning(self, c):
        if self.model.conditioning_key == 'crossattn_layerwise':
            c_all = []
            for i in range(16):
                c_i = [t.replace(self.placeholder_token, self.layer_tokens[i]) for t in c]
                if self.cond_stage_forward is None:
                    if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                        c_i = self.cond_stage_model.encode(c_i, embedding_manager=self.embedding_manager)
                        if isinstance(c, DiagonalGaussianDistribution):
                            c_i = c_i.mode()
                    else:
                        c_i = self.cond_stage_model(c_i)
                else:
                    assert hasattr(self.cond_stage_model, self.cond_stage_forward)
                    c_i = getattr(self.cond_stage_model, self.cond_stage_forward)(c_i)
                c_all.append(c_i)
            c = torch.stack(c_all, dim=1)   # B x 16 x 77 x 768
        else:
            if self.cond_stage_forward is None:
                if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.embedding_density_reg_weight > 0:
            token_embeddings = self.cond_stage_model.transformer.text_model.embeddings.token_embedding.weight.clone()
            loss_embedding_density_reg = self.embedding_manager.embedding_to_density_loss(token_embeddings).mean()

            loss_dict.update({f'{prefix}/loss_emb_density_reg': loss_embedding_density_reg})

            loss += (self.embedding_density_reg_weight * loss_embedding_density_reg)
            loss_dict.update({f'{prefix}/loss': loss})
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'crossattn_layerwise', 'hybrid', 'adm']
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        ...
        elif self.conditioning_key == 'crossattn' or self.conditioning_key == 'crossattn_layerwise':
